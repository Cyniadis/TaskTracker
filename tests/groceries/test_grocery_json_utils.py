...
class TestGroceryListRoundTrip:
    def test_a_bought_item_stays_bought_after_save_and_load(self, groceries_file):
        item = GroceryItem(name="Lait", state=GroceryState.BOUGHT.value, last_bought_date=date(2026, 7, 26))
    ...
    def test_saving_overwrites_the_previous_contents_entirely(self, groceries_file):
        grocery_json_utils.save_groceries(
            [GroceryItem(name="First"), GroceryItem(name="Second")], path=groceries_file,
        )
        grocery_json_utils.save_groceries([GroceryItem(name="Only this one now")], path=groceries_file)
