"""Pure-domain tests for tasktracker.groceries.grocery — no Streamlit, no I/O."""
from datetime import date

from tasktracker.groceries.grocery import GroceryItem, GroceryState, STATE_LABELS
...
def test_new_item_defaults_to_to_buy():
    ...
    assert item.state_label == STATE_LABELS[GroceryState.TO_BUY]
...
def test_set_state_from_label_to_bought_stamps_last_bought_date():
    ...
    item.set_state_from_label(STATE_LABELS[GroceryState.BOUGHT], today)
...
def test_set_state_from_label_to_not_to_buy_leaves_last_bought_date_untouched():
    ...
    item.set_state_from_label(STATE_LABELS[GroceryState.BOUGHT], bought_date)

    item.set_state_from_label(STATE_LABELS[GroceryState.NOT_TO_BUY], date(2026, 7, 27))
...
def test_restore_reverts_to_original_snapshot():
    item = GroceryItem(id=0, name="Lait")
    item.set_field("name", "Lait demi-écrémé")
    item.set_state_from_label(STATE_LABELS[GroceryState.BOUGHT], date(2026, 7, 27))
